...
N_TRIALS = 1#100  # Maximum number of trials
N_JOBS = 1 # Number of jobs to run in parallel
N_STARTUP_TRIALS = 5  # Stop random sampling after N_STARTUP_TRIALS
N_EVALUATIONS = 0#1  # Number of evaluations during the training
# N_TIMESTEPS = int(2e4)  # Training budget
EVAL_FREQ = 3000#50#int(N_TIMESTEPS / N_EVALUATIONS)
# N_EVAL_ENVS = 5
N_EVAL_EPISODES = 1#
...
def objective(trial: optuna.Trial) -> float:
    ...
    eval_freq = agent_cfg.max_iterations#EVAL_FREQ
...
def main():
    ...
    try:
        study.optimize(objective, n_trials=N_TRIALS, n_jobs=N_JOBS, gc_after_trial=True, timeout=TIMEOUT)
