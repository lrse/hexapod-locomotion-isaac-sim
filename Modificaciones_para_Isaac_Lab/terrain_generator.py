...
class TerrainGenerator:
    ...
    def __init__(self, cfg: TerrainGeneratorCfg, device: str = "cpu"):
        ...
        '''Agregado por Tania'''
        self.terrain_parameter = np.zeros((self.cfg.num_rows, self.cfg.num_cols))
        '''Fin Agregado por Tania'''
    ...
    def _generate_curriculum_terrains(self):
        ...
        sub_terrains_cfgs = list(self.cfg.sub_terrains.values())
        '''Agregado por Tania'''
        sub_terrains_types = list(self.cfg.sub_terrains.keys())
        '''Fin Agregado por Tania'''
        ...
        for sub_col in range(self.cfg.num_cols):
            for sub_row in range(self.cfg.num_rows):
                ...
                '''Agregado por Tania'''
                terrain_type = sub_terrains_types[sub_indices[sub_col]]
                cfg = sub_terrains_cfgs[sub_indices[sub_col]]
                match terrain_type:
                    case "pyramid_stairs":
                        step_height = cfg.step_height_range[0] + difficulty * (cfg.step_height_range[1] - cfg.step_height_range[0])
                        self.terrain_parameter[sub_row, sub_col] = step_height
                    case "pyramid_stairs_inv":
                        step_height = cfg.step_height_range[0] + difficulty * (cfg.step_height_range[1] - cfg.step_height_range[0])
                        self.terrain_parameter[sub_row, sub_col] = step_height
                    case "boxes":
                        grid_height = cfg.grid_height_range[0] + difficulty * (cfg.grid_height_range[1] - cfg.grid_height_range[0])
                        self.terrain_parameter[sub_row, sub_col] = grid_height
                    case "random_rough":
                        pass
                    case "hf_pyramid_slope":
                        slope = cfg.slope_range[0] + difficulty * (cfg.slope_range[1] - cfg.slope_range[0])
                        self.terrain_parameter[sub_row, sub_col] = slope
                    case "hf_pyramid_slope_inv":
                        slope = cfg.slope_range[0] + difficulty * (cfg.slope_range[1] - cfg.slope_range[0])
                        self.terrain_parameter[sub_row, sub_col] = slope
                    case _:
                        raise ValueError(f"Invalid terrain type for this test: {terrain_type}.")
                '''Fin Agregado por Tania'''
                # add to sub-terrains
